...
def per_image_standardization(x):
  ...
  N = x_s[1]*x_s[2]*x_s[3]
  mean = math_ops.reduce_mean(x, [-1, -2, -3], keepdims=True)
  std = tf.math.reduce_std(x, [-1, -2, -3], keepdims=True)

  ajust_std = math_ops.maximum(std, math_ops.rsqrt(float(N)))
  x = (x-mean)/ajust_std
  return x
...
class Model(object):
  ...
  def _build_model(self, image, label):
    ...
    with tf.variable_scope('input'):
      ...
      #                         self.x_input)
      x = per_image_standardization(self.x_input)
    
    
    
    with tf.variable_scope('input'):
      x = self._conv('init_conv', x,
