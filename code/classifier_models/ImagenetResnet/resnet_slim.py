...
def bottleneck(inputs,
  ...
  with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
    ...
    if depth == depth_in:
      ...
      shortcut_record = shortcut
      shortcut = tf.nn.relu6(shortcut) if use_bounded_activations else shortcut
    ...
    if config.config["resnet_record_mode"] in ["None", "Combine"]:
      resnet_output = tf.concat([residual, shortcut_record],  axis=3)
    elif config.config["resnet_record_mode"] in ["Main"]:
      resnet_output = residual
    elif config.config["resnet_record_mode"] in ["Shortcut"]:
      resnet_output = shortcut_record
    config.config["resnet_record"] = resnet_output
